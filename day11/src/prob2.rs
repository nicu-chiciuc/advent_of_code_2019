...
fn print_image(colors: &HashMap<Point, i64>) {
  for y in (-50..50).rev() {
    for x in -50..50 {
      let col = colors.get(&Point { x, y }).cloned().unwrap_or(0);
...
pub fn main() {
  ...
  // For problem 2, first color is white
  color_white.insert(Point { x: 0, y: 0 }, 1);
